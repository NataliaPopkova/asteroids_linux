...
int main(int, const char**) {
    ...
    window =
        XCreateWindow(display, DefaultRootWindow(display), 10, 10, SCREEN_WIDTH,
                      SCREEN_HEIGHT, 1, 24, InputOutput, CopyFromParent, 0, 0);
    ...
    XSetWMProperties(display, window, NULL, NULL, NULL, 0, sizehints, wmhints,
                     classhint);
    ...
    XImage* image = XCreateImage(display, visual, 24, ZPixmap, 0, (char*)buffer,
    ...
    for (;;) {
        ...
        while (XPending(display)) {
            ...
            if (event.type == ClientMessage &&
                event.xclient.data.l[0] == (int)wmDeleteMessage)
                quit = true;
        }
        ...
        draw();
        XPutImage(display, pixmap, gc, image, 0, 0, 0, 0, image->width,
                  image->height);
        XCopyArea(display, pixmap, window, gc, 0, 0, SCREEN_WIDTH,
                  SCREEN_HEIGHT, 0, 0);
        XFlush(display);
    }
